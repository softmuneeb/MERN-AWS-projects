...
// event Mint(address indexed sender, uint amount0, uint amount1);
// Method 1, https://docs.ethers.io/v5/api/providers/provider/#Provider--events
const mint = pair.filters.Mint();
provider.on(mint, (sender, amount0, amount1) => {
  console.log({ sender, amount0, amount1 });
});

// Method 2, https://ethereum.stackexchange.com/questions/87643/how-to-listen-to-contract-events-using-ethers-js
...
// event Burn(address indexed sender, uint amount0, uint amount1, address indexed to);
// Method 1
const burn = pair.filters.Burn();
provider.on(burn, (sender, amount0, amount1, to) => {
  console.log({ sender, amount0, amount1, to });
});
...
// event PairCreated(address indexed token0, address indexed token1, address pair, uint);
// Method 1
const pairCreated = factory.filters.PairCreated();
provider.on(pairCreated, (sender, amount0, amount1, to) => {
  console.log({ sender, amount0, amount1, to });
});

// Method 2
factory.on('PairCreated', (sender, amount0, amount1, to) => {
  console.log({ sender, amount0, amount1, to });
});
